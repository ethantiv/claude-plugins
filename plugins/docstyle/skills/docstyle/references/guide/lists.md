...
Tables and lists are both ways to present a set of similarly structured items. Sometimes it's not obvious when to choose one presentation over the other. To decide which presentation to use, see [List or table?](/style/tables#list-or-table)
...
See also [Sub-steps in numbered procedures](/style/procedures#sublists). | `ol`, `li`  
...
Introduce a list with a complete sentence, not a partial one that's completed by the list items. You can also use _the following_ as a noun phrase (see [following](/style/word-list#following) in the word list).
...
For information about sub-steps in a numbered procedure, see [Procedures](/style/procedures#sublists).
...
Don't use a dash to set off a description from an item in a description list. For more information, see [Colons instead of dashes in lists](/style/dashes#colons-instead-of-dashes-in-description-lists).
...
When you write a list in a paragraph, use [serial commas](/style/commas#serial-commas) to separate the items. 
