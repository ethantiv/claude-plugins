...
In ordinary text sentences (as opposed to, say, [code samples](/style/code-samples)), use code font to mark up most things that have anything to do with code. Code font helps to clarify for your reader which text refers to an entity in these ways:
...
For information about choosing HTML or Markdown, see [Markdown versus HTML](/style/markdown).
...
[UI elements](/style/ui-elements) that are rendered based on previously entered text (such as a server or instance name) | From the **Server name** list, select **`my-sql-cluster1`**. Click **`my-instance`**. If a code-formatted element appears in UI, add bold as well. For more information, see Code in UI elements.  
...
If a [UI element](/style/ui-elements#formatting) meets the requirements for code font, then use both code font and bold for that element.
...
In particular, use _N xx_ (with a specific digit in place of N) to indicate _anything in the N00-N99 range_, and put the status code number in code font even if you're leaving out the code's name.
