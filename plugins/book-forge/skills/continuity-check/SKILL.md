...
- To **jedyny** etap zapisujący do kanonu. Etapy pisania tylko proponowały; tu propozycje są weryfikowane i dopiero teraz stają się kanonem.
...
2. **Klasyfikacja** — synteza zbiera audyty i każdą propozycję/ustalenie klasyfikuje jako **CONFLICT** (łamie RO), **WRITE** (nowy RUNTIME) lub **OK** (zgodne istniejące). Tworzy **plan zapisu** (tylko RUNTIME) oraz wpis do `log_ciaglosci` (PASS/CONFLICT/FIX).
