...
Skopiuj do narzędzia **Workflow**. `args`: `{ idea, genre, reader, pov, czas, subgenre, tomy, outline, existing }`, gdzie `idea` to zwycięski pomysł z etapu 1, `outline` to streszczenie konspektu, a `existing` to już istniejąca biblia (lub null przy pierwszym uruchomieniu — idempotencja). Kształt zwracanego obiektu odpowiada `${CLAUDE_PLUGIN_ROOT}/shared/biblia-spec.md`.
...
const [swiat, post, glos, glosar, temat] = await parallel([
  ...
  ()=>agent(`${ROLE}\n\nROLA: twórca postaci. Zbuduj POSTACIE (want/need/rana/kłamstwo/łuk, opis fizyczny, relacje, odmiana imienia), ANTAGONISTĘ (cel, motywacja „ma rację z własnej perspektywy”, plan, przewaga) i STAWKĘ (osobista, globalna, zegar).`,{label:'postacie',phase:'Sekcje',schema:S_POST}),
